...
const transform: TypeGenFn = (gen, classInfo, version, options) => {
    ...
    if (
        classInfo.type === 'java.util.HashMap' ||
        classInfo.type === 'java.util.Map'
    ) {
    ...
    if (generic && generic.length === 2) {
        ...
        gen(
            "    const encodeKey = compile('%s', %j, classMap, version, %j); encodeKey(key, encoder, appClassMap);",
            keyId,
            genericKeyDefine,
            options,
        )
        gen(
            "    const encodeValue = compile('%s', %j, classMap, version, %j); encodeValue(value, encoder, appClassMap);",
            valueId,
            genericValueDefine,
            options,
        )
        ...
        if (convertor) {
            gen(
                "    const encodeKey = compile('%s', %j, classMap, version, %j); encodeKey(%s(key), encoder, appClassMap);",
                keyId,
                genericKeyDefine,
                options,
                convertor,
        ...
        gen(
            '      compile(fnKey, value, appClassMap, version, %j)(value.$, encoder);',
            options,
        )
        gen('    } else {')
