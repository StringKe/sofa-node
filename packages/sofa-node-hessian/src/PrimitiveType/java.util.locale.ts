...
const transform: TypeGenFn = (gen, classInfo, version) => {
    ...
    if (version === '1.0') {
        ...
        gen(
            "const ref = encoder._writeObjectBegin('com.caucho.hessian.io.LocaleHandle');",
        )
        gen('if (ref === -1) {')
        gen('encoder.writeInt(1);')
        gen("encoder.writeString('value');")
        gen(
            "encoder._writeObjectBegin('com.caucho.hessian.io.LocaleHandle'); }",
        )
        gen('encoder.writeString(obj);')
    }
}
