...
const transform: TypeGenFn = (gen, classInfo, version, options) => {
    ...
    if (version === '1.0') {
        ...
        gen(
            "compile('%s', %j, classMap, version, %j)(obj, encoder);",
            uniqueId,
            attr,
            options,
        )
        ...
        gen(
            "const ref = encoder._writeObjectBegin('java.util.concurrent.atomic.AtomicLong');",
        )
        gen('if (ref === -1) {')
        gen('encoder.writeInt(1);')
        gen("encoder.writeString('value');")
        gen(
            "encoder._writeObjectBegin('java.util.concurrent.atomic.AtomicLong'); }",
        )
        gen(
            "compile('%s', %j, classMap, version, %j)(obj, encoder);",
            uniqueId,
